...
class TestActivation(unittest.TestCase):
    ...
    def _test_activation(cls, fn):
        ...
        x = torch.linspace(-np.pi, np.pi, 64, dtype=torch.float64).to(device)
        eps = 1e-6
...
class TestSiren(unittest.TestCase):
    def test_derivative(self):
        ...
        x = torch.rand(64, dtype=torch.float64).to(device)
        y = torch.rand(64, dtype=torch.float64).to(device)
        eps = 1e-6
